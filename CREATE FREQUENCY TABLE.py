...
def cumulative_absolute_frequency():
    ...
    for x in range(1,n_intervals_round+1):
        ...
        if x == n_intervals_round:
            table[x][3] = color.GREEN + str(int(table[x][2]) + cumulative_sum) + color.ENDC
        else:
            table[x][3] = table[x][2] + cumulative_sum
            cumulative_sum = table[x][3]
...
def cumulative_relative_frequency():
    ...
    for x in range(1,n_intervals_round+1):
        ...
        if x == n_intervals_round:
            table[x][5] = color.GREEN + str(table[x][4] + relative_sum) + color.ENDC
        else:
            table[x][5] = round(table[x][4] + relative_sum,2)
            relative_sum = table[x][5]
#----------------------------------------------------XI * fi------------------------------------------------------------
def Xi_fi():
    for x in range(1,n_intervals_round+1):
